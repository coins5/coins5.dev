...
When rendering a list of items (e.g., transactions, patient records, products) along with their related entities, an unoptimized ORM will execute one query to fetch the main list, and then another individual query for _each_ item in that list to fetch its relations.
